...
class QLearner(nn.Module):
    ...
    def act(self, state, epsilon):
        if random.random() > epsilon:
            ...
            action = self(state).detach().max(1)[1]
...
def compute_td_loss(model, target_model, batch_size, gamma, replay_buffer):
    ...
    y = reward + gamma * target_model(next_state).detach().max(1)[0] * (1-done)
    
    loss = torch.nn.MSELoss(reduction='sum')(y, model(state).gather(1, action.unsqueeze(-1)).squeeze(-1))
    return loss
